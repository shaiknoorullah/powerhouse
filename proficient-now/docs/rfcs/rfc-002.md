---
type: rfc
modified: 2025-05-27T19:54:53+05:30
...
The Cilium CNI architecture, as previously discussed in conceptual documents (), involves agents on each node interacting with the Linux kernel via eBPF, managed by an operator.
...
### 5.4. Load Balancing with MetalLB (Internal)

MetalLB provides `LoadBalancer` type services for exposing applications _internally_ within your cluster's network segments, or for providing stable internal endpoints that OPNsense can NAT public traffic to.

**Mode Selection:** As per your query, MetalLB will be configured in **BGP mode**, peering with OPNsense.
...
**2. `BGPPeer` CRD (to peer with OPNsense):**

```
# metallb-bgppeer-opnsense.yaml
apiVersion: metallb.io/v1beta1
kind: BGPPeer
metadata:
  ...
  namespace: metallb-system
spec:
  myAsn: 64512 # ASN for MetalLB/Kubernetes Cluster
  peerAsn: 64513 # ASN for OPNsense (FRR)
  peerAddress: 192.168.102.1 # OPNsense IP on the K8s Internal VLAN (VLAN 102)
  # Optional: sourceAddress if MetalLB needs to use a specific K8s node IP for BGP session
  # nodeSelectors: # If you want only specific nodes to run BGP speakers
  # - matchLabels:
  #     kubernetes.io/os: linux
```

_Note: OPNsense (via FRR) must be configured with the corresponding ASN (64513) and to peer with MetalLB's ASN (64512) and the K8s node IPs where speaker pods run._

**3.** `BGPAdvertisement` CRD (to **announce IPs from the pool):**

```
# metallb-bgpadvertisement.yaml
apiVersion: metallb.io/v1beta1
kind: BGPAdvertisement
metadata:
  name: k8s-internal-services-advertisement
  namespace: metallb-system
spec:
  ipAddressPools:
  - k8s-internal-services-pool
  aggregationLength: 32 # Announce individual /32s
  # aggregationLengthV6: 128
  # localPref: 100 # Optional BGP local preference
  # communities: ["65535:65282"] # Optional BGP communities
```

These CRDs would be deployed via ArgoCD after MetalLB itself is installed. The `k8s-deployment-plan.md` referenced deploying MetalLB via its Helm chart, which is a good approach. These CRDs would then be applied as separate manifests or as part of the Helm values if supported.
...
- MetalLB (in BGP mode) will assign it an IP from the `192.168.102.200-192.168.102.250` pool (e.g., `192.168.102.200`).
...
(Content from Pn-Infra Networking.md Part 2.5 is relevant).
...
_Ensure_ the CoreDNS deployment is patched _to use this custom configuration._

## Table of Contents (Continued)

5.  [Kubernetes Networking Configuration](#kubernetes-networking-configuration)
    5.1. [CNI - Cilium](#cni---cilium)
        5.1.1. [Cilium Architecture and Deployment Review](#cilium-architecture-and-deployment-review)
        5.1.2. [Cilium Configuration (eBPF, Hubble, WireGuard)](#cilium-configuration-ebpf-hubble-wireguard)
    5.2. [Pod and Service CIDRs Review](#pod-and-service-cidrs-review)
    5.3. [VLAN Assignment for Kubernetes Nodes Review](#vlan-assignment-for-kubernetes-nodes-review)
    5.4. [Load Balancing with MetalLB (Internal - L2 Mode)](#load-balancing-with-metallb-internal---l2-mode)
    5.5. [Ingress Controller - Nginx](#ingress-controller---nginx)
    5.6. [Certificate Management - Cert Manager](#certificate-management---cert-manager)
    5.7. [Network Policies with Cilium](#network-policies-with-cilium)
    5.8. [Ceph Storage Networking](#ceph-storage-networking)
    5.9. [Nested Cluster Networking (Kube-OVN & VyOS - Optional)](#nested-cluster-networking-kube-ovn--vyos---optional)
    5.10. [DNS and Service Discovery (CoreDNS)](#dns-and-service-discovery-coredns)
6.  [Advanced Network Security and Routing via OPNsense](#advanced-network-security-and-routing-via-opnsense)
    6.1. [OPNsense Firewall Rule Design](#opnsense-firewall-rule-design)
    6.2. [Advanced Routing with OPNsense (Policy-Based Routing)](#advanced-routing-with-opnsense-policy-based-routing)
    6.3. [Traffic Shaping and QoS on OPNsense](#traffic-shaping-and-qos-on-opnsense)
    6.4. [Intrusion Detection/Prevention (Suricata on OPNsense)](#intrusion-detectionprevention-suricata-on-opnsense)
7.  [Network Observability and Monitoring](#network-observability-and-monitoring)
    7.1. [Observability Stack Architecture](#observability-stack-architecture)
    7.2. [Prometheus Configuration for Network Components](#prometheus-configuration-for-network-components)
    7.3. [Hubble for Network Flow Monitoring](#hubble-for-network-flow-monitoring)
    7.4. [Log Collection and Analysis (Loki & Promtail) for Network Events](#log-collection-and-analysis-loki--promtail-for-network-events)
    7.5. [Visualization Dashboards (Grafana) for Network Metrics](#visualization-dashboards-grafana-for-network-metrics)
    7.6. [Network Alerting](#network-alerting)
8.  [GitOps for Infrastructure and Network Configuration](#gitops-for-infrastructure-and-network-configuration)
    8.1. [ArgoCD for GitOps (Kubernetes Resources)](#argocd-for-gitops-kubernetes-resources)
    8.2. [Repository Structure for Network Configurations](#repository-structure-for-network-configurations)
    8.3. [CI/CD Pipeline for Network Policy Validation](#cicd-pipeline-for-network-policy-validation)
    8.4. [Managing OPNsense Configuration with GitOps (Conceptual)](#managing-opnsense-configuration-with-gitops-conceptual)
9.  [Ongoing Maintenance and Best Practices](#ongoing-maintenance-and-best-practices)
    9.1. [Upgrade Procedures for Network Components](#upgrade-procedures-for-network-components)
    9.2. [Backup and Recovery for Network Infrastructure](#backup-and-recovery-for-network-infrastructure)
    9.3. [Network Security Audits and Health Checks](#network-security-audits-and-health-checks)
    9.4. [Disaster Recovery for Network Services](#disaster-recovery-for-network-services)
10. [Appendix](#appendix)
    10.1. [Consolidated Network Configuration Data (Reference)](#consolidated-network-configuration-data-reference)
    10.2. [Key Scripts and Manifests (Placeholders/Examples)](#key-scripts-and-manifests-placeholdersexamples)

---
*(Continuing from Stage 4 of the previous document)*
...
_Ensure_ the CoreDNS deployment _is patched to use this custom configuration._
...
1. **WAN** Interface (e.g., **`vtnet0` on VLAN 100):**
    ...
    - **Port** Forwarding (NAT) for **Kubernetes Services:**
        
        - For each public IP and port combination needed for an external service (e.g., `103.110.174.17:443` for HTTPS):
            
            - Create a NAT Port Forward rule to translate the destination to the Nginx Ingress controller's internal IP (e.g., `192.168.102.200:443`) assigned by MetalLB on VLAN 102.
                
            - Ensure the associated filter rule is automatically created or manually add a WAN interface rule to `pass` this specific forwarded traffic.
                
            - Example: `Pass` TCP Any -> WAN Address:443 (NAT to `192.168.102.200:443)`
...
3. **K8S_INTERNAL Interface (e.g., `vtnet2` on VLAN 102 - `192.168.102.0/24`):**
    ...
    - **Allow Outbound to WAN (NATed):** Allow traffic from the K8s pod CIDR (`10.42.0.0/16`) and K8s node IPs on VLAN 102 (`192.168.102.0/24`) to `any` on the WAN interface. Outbound NAT should be configured for these sources on OPNsense.
        
        - Consider FQDN alias-based rules for allowing egress to specific external services (e.g., container registries, external APIs) for better security.
...
5. **STORAGE_PRIV** Interface (e.g., `vtnet4` on **VLAN 104 - `192.168.104.0/24`):**
    ...
    - **Strictly Internal:** This network should generally _not_ have direct internet access or access to other non-storage related VLANs.
        
    - Allow traffic only between Ceph nodes on this VLAN for replication and heartbeats. OPNsense rules here would be to block any unintended traffic trying to route _out_ of this segment.
...
1. **Pipes** (Firewall -> Shaper -> Pipes): Define the maximum bandwidth available.
    
    - Create an upload pipe and a download pipe for your WAN connection(s).
        
2. **Queues (Firewall -> Shaper -> Queues):** Define how bandwidth within a pipe is distributed.
    
    - Use schedulers like `FQ_CODEL` (Fair Queueing with Controlled Delay) for good general performance.
        
    - Create child queues under pipes for different traffic priorities (e.g., High_Priority_K8S, Default_K8S, Low_Priority_Bulk).
        
    - Assign weights or bandwidth percentages to these queues.
        
3. **Rules (Firewall -> Shaper -> Rules):** Assign traffic to specific queues.
    
    - Create rules matching traffic (e.g., by source/destination IP, port, DSCP value) and assign it to a queue.
...
1. **Installation (Services -> Intrusion Detection -> Administration -> Install Suricata):** Install if not already present.
...
3. **Interfaces** (Services -> Intrusion Detection -> **Administration -> Interfaces):**
    
    - Assign interfaces for Suricata to monitor (e.g., WAN, K8S_INTERNAL - VLAN 102, MANAGEMENT - VLAN 106).
        
    - Enable IPS mode per interface if you decide to use it.
        
4. **Rule** Sets (Services -> **Intrusion Detection -> Administration -> Rules / Download):**
    
    - Enable and download reputable rule sets (e.g., **ET Open** is free; Snort VRT requires subscription).
        
    - Schedule automatic updates for rule sets (e.g., daily).
        
5. **Policies (Services -> Intrusion Detection -> Administration -> Policies):**
    
    - Define policies to select which categories of rules from your downloaded sets are active. Start with a balanced policy (e.g., one that prioritizes known exploits and malware C&C) and tune based on alerts and your environment's specific risk profile.
        
6. **Alerts (Services -> Intrusion Detection -> Alerts):** Monitor alerts generated by Suricata.
    
7. **Logging:** Ensure Suricata logs are sent to OPNsense system logs and subsequently to Loki/Promtail (see Stage 7.4). The `eve.json` log format is rich and useful for SIEM integration.
    
8. **Tuning:** IDS/IPS requires ongoing tuning. Regularly review alerts to identify false positives and suppress or modify problematic rules. Understand the impact of rules before enabling them in blocking (IPS) mode.
...
- **OPNsense Log Forwarding:**
    ...
    - Configure OPNsense (System -> Settings -> Logging / Syslog) to send logs to Promtail's syslog listener IP/port (e.g., a Promtail service IP on the management VLAN 106, port `1514`).
        
    - Forward logs for: System events, Firewall (filterlog), Suricata, OpenVPN, DHCP, DNS (Unbound/Dnsmasq).
...
### 10.1. Consolidated Network Configuration Data (Reference)

| Item | VLAN ID | Network | Gateway (OPNsense as GW for VMs) | Key IPs |